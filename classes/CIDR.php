...
class CIDR {
	 ...
	 */
	public static function cidrToRange($cidr,$long=false) {
		$range = array();
		$cidr = explode('/', $cidr);
		...
		return $long ? [$start,$end] : [long2ip($start), long2ip($end)];
	}

	/**
	 * method iterate.
	 * Excecutes callback for each address in a CIDR block
	 * Usage:
	 *     CIDR::iterate('192.168.1.0/24',$function);
	 * @param $cidr string CIDR block
	 * @param $function anonymous callback function
	 */
	public static function iterate($cidr,$function) {
		list($start,$end) = self::cidrToRange($cidr,true);
		for($i = $start; $i <= $end; $i++) {
			$function(long2ip($i));
		}
	}
